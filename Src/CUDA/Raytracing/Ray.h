...
__device__ float3 ray_origin_epsilon_offset(float3 origin, float3 direction, float3 geometric_normal) {
	return origin + sign(dot(direction, geometric_normal)) * EPSILON * geometric_normal;
}

__device__ float3 reflect_direction(float3 direction, float3 normal) {
	return 2.0f * dot(direction, normal) * normal - direction;
}

__device__ float3 refract_direction(float3 direction, float3 normal, float eta) {
