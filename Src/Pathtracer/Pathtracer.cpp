...
void Pathtracer::cuda_init(unsigned frame_buffer_handle, int screen_width, int screen_height) {
	...
	first_frame_after_stopped_updating = true;
...
void Pathtracer::cuda_free() {
	...
	if (scene.textures.size() > 0) {
		...
		for (int i = 0; i < scene.textures.size(); i++) {
			/*for (int l = 0; l < scene.textures[i].mip_levels; l++) {
				CUarray level_array;
				CUDACALL(cuMipmappedArrayGetLevel(&level_array, tex_arrays[i], l));

				CUDAMemory::free_array(level_array);
			}*/
...
void Pathtracer::resize_init(unsigned frame_buffer_handle, int width, int height) {
	...
	// Allocate GBuffers
	array_gbuffer_normal_and_depth        = CUDAMemory::create_array(pitch, height, 4, CU_AD_FORMAT_FLOAT);
	array_gbuffer_mesh_id_and_triangle_id = CUDAMemory::create_array(pitch, height, 2, CU_AD_FORMAT_SIGNED_INT32);
	array_gbuffer_screen_position_prev    = CUDAMemory::create_array(pitch, height, 2, CU_AD_FORMAT_FLOAT);
	...
	ptr_frame_buffer_albedo = CUDAMemory::malloc<float4>(pitch * height);
	ptr_frame_buffer_moment = CUDAMemory::malloc<float4>(pitch * height);
	...
	ptr_history_length           = CUDAMemory::malloc<int>   (pitch * height);
	ptr_history_direct           = CUDAMemory::malloc<float4>(pitch * height);
	ptr_history_indirect         = CUDAMemory::malloc<float4>(pitch * height);
	ptr_history_moment           = CUDAMemory::malloc<float4>(pitch * height);
	ptr_history_normal_and_depth = CUDAMemory::malloc<float4>(pitch * height);
	...
	ptr_taa_frame_prev = CUDAMemory::malloc<float4>(pitch * height);
	ptr_taa_frame_curr = CUDAMemory::malloc<float4>(pitch * height);
